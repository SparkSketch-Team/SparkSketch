...
public class UserSummary
{
    ...
    //{
    //    var db = new SparkSketchContext();
    //    var userTypeClaim = getClaim(httpContext, SparkSketchClaims.UserType);
    ...
    //    var userId = getClaim(httpContext, SparkSketchClaims.UserId);
    //    var user = await db.Users.Where(x => x.Username).FirstOrDefaultAsync(x => x.UserId.ToString() == (userId && x.IsActive));
